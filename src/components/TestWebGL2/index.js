...
const TestWebGL = props => {
    ...
    const initThree = () => {
        ...
        function initModel() {
            //轴辅助 （每一个轴的长度）
            const object = new THREE.AxesHelper(10000);
            // scene.add(object);
        }
        function initObject() {
            //加载贴图
        ...
        function initJSON() {
            ...
            for (var i = 0; i < json.length; i++) {
                if (json[i].R == 193 && json[i].G == 205 && json[i].B == 205) {
                    cnt++;
                    continue;
                } else {
                    // positions
                    ...
                    // colors
                    const R = json[i].R / 255;
                    const G = json[i].G / 255;
                    const B = json[i].B / 255;
            ...
            geometry.rotateY(-80.2);
            geometry.translate(190, -25, 147)
            const material = new THREE.PointsMaterial({ size: 1.5, vertexColors: THREE.VertexColors });
